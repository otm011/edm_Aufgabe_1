...
bool check_graph() {
    ...
    for(unsigned long int i=0;i<graph.edge_count;i++){
        unsigned long int a=graph.edges[i].a;
        unsigned long int b=graph.edges[i].b;
        Union(a,b);
    }
    unsigned long int start=find(0);
    for (unsigned long int i=0;i<graph.vertex_count;i++){
        if (find(i)!=start){
            return false;
        }
    }
...
void union_find() {
    ...
    for(unsigned long int i=0;i<graph.edge_count;i++){
        unsigned long int a=graph.edges[i].a;
        unsigned long int b=graph.edges[i].b;
        unsigned long int fa = find(a);
        unsigned long int fb = find(b);
        if (fa != fb){
...
void ausgeben(Graph &result) {
    ...
    for (unsigned long int i=0; i < result.edge_count; i++){
